<!-- ----------------------------------- PRIORITY + PREMIUM LISTED ITEMS :  -->
<?php
$priorityCMD = $pdo->prepare("SELECT product.*,
        product_enhancements.priority_listing, product_enhancements.premium_listing, product_enhancements.status, product_enhancements.highlighted_listing
        FROM product
        INNER JOIN product_enhancements
        ON product.product_id = product_enhancements.product_id
        WHERE product_enhancements.priority_listing = 1 
        OR product_enhancements.premium_listing = 1
        AND product_enhancements.status  = 'active'");
$priorityCMD->execute();
$priorityItems = $priorityCMD->fetchAll(PDO::FETCH_ASSOC);
...
?>
    <div class="premium-listing">
        <div class="line-with-text">
            <h2 class="text">Priority Listed + Premium Items</h2>
            <div class="line"></div>
        </div>
            ...
            ?>
                <div class="items">
                    ...
                    if ($row['stock_qnty'] > 0 && !in_array($row['category_id'], [4, 5, 11])) { ?>
                        <div class="i-btn">
                            <button id="btn_<?php echo $row['product_id']; ?>"
                                onclick="addToCart(
