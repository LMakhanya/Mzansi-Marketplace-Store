<!-- ----------------------------------- CATEGORY LISTED :  -->
<?php
$categoryFeaturedCMD = $pdo->prepare("SELECT product.*,
    ...
    WHERE 
    product_enhancements.category_featured = 1
    AND
    product_enhancements.status  = 'active'");
$categoryFeaturedCMD->execute();
$categoryFeaturedItems = $categoryFeaturedCMD->fetchAll(PDO::FETCH_ASSOC);

// Check if fewer than 20 items and fetch random items if needed
if (count($categoryFeaturedItems) < 20) {
    $remaining = 20 - count($categoryFeaturedItems);
    $excludeIds = array_column($categoryFeaturedItems, 'product_id');
    ...
    // Merge category featured and random items
    // $categoryFeaturedItems = array_merge($categoryFeaturedItems, $randomItems);
}


if (!empty($categoryFeaturedItems)):
?>
    <div class="category-listing">
        <div class="line-with-text">
            <h2 class="text">Category Featured Items</h2>
            <div class="line"></div>
        </div>
        ...
        <div class="i-wraper ">
            ...
            <?php
            foreach ($categoryFeaturedItems as $row) {
